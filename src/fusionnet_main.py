...
def train(train_image_path,
    ...
    for paths in [train_depth_paths, train_ground_truth_paths, train_lidar_map_paths]:
    ...
    for paths in [val_depth_paths, val_ground_truth_paths]:
    ...
    for epoch in range(1, learning_schedule[-1] + 1):
        ...
        for batch_data in train_dataloader:
            ...
            # Unpack data (移除response)
            image, depth, ground_truth, lidar_map = batch_data

            # Apply augmentations and data transforms
            [image], [depth, ground_truth, lidar_map] = train_transforms.transform(
                images_arr=[image],
                range_maps_arr=[depth, ground_truth, lidar_map],
                random_transform_probability=augmentation_probability)

            # 输入仅使用depth，移除response拼接
            input_depth = depth
            ...
            validity_map_loss_smoothness = torch.where(
                # ground_truth > 0,
                (ground_truth > 0) & (ground_truth < 20.0),
            ...
            if (train_step % n_step_per_summary) == 0:
                ...
                with torch.no_grad():
                    # Log tensorboard summary (移除input_response)
...
def validate(model,
    ...
    input_depth_summary = []
    ground_truth_summary = []  # 移除response_summary
    ...
    for idx, inputs in enumerate(dataloader):
        ...
        # 解压数据时移除response
        image, depth, ground_truth = inputs
        ...
        if (idx % n_summary_display_interval) == 0 and summary_writer is not None:
            ...
            ground_truth_summary.append(ground_truth)  # 移除response_summary相关
        ...
        validity_map = np.where((ground_truth > 0) & (ground_truth < 20.0), 1, 0)
    ...
    # Log to tensorboard (移除input_response)
...
def run(restore_path,
        image_path,
        depth_path,
        ground_truth_path,  # 移除response_path参数
        # Input settings
        input_channels_image,
        input_channels_depth,
        normalized_image_range,
    ...
    # 移除response_paths校验
    assert n_sample == len(depth_paths)
    if ground_truth_available:
        assert n_sample == len(ground_truth_paths)

    # 数据集加载移除response_paths参数
    ...
    with torch.no_grad():
        ...
        for idx, data in enumerate(dataloader):
            ...
            if save_outputs:
                ...
                # Create output paths (移除response相关)
