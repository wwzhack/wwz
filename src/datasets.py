...
class FusionNetTrainingDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        for paths in [depth_paths, ground_truth_paths, lidar_map_paths]:
    ...
    def __getitem__(self, index):
        ...
        if self.do_random_crop:
            [image, depth, ground_truth, lidar_map] = random_crop(
                inputs=[image, depth, ground_truth, lidar_map],
                shape=self.shape,
                crop_type=self.random_crop_type)

        # Convert to float32
        image, depth, ground_truth, lidar_map = [
            T.astype(np.float32)
            for T in [image, depth, ground_truth, lidar_map]
        ]

        return image, depth, ground_truth, lidar_map
